...
const tabs = [
  ...
  // { label: "Voice & Video", value: "voice_video" },
];

const SidebarRight = () => {
  const [indexActive, setIndexActive] = useState("about");

  return (
    <aside className="p-4 border-l border-l-gray-200 w-80 h-full flex flex-col gap-4">
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <div className="font-medium text-xl">Mirabelle Tow</div>
          ...
          {tabs.map((tab) => (
            <li key={tab.value}>
              <button
                className={clsx([
                  `font-medium pb-2`,
                  indexActive === tab.value
                    ? `text-blue-500 border-b-blue-500 border-b-2`
                    : `text-gray-500`,
                ])}
                onClick={() => setIndexActive(tab.value)}
              >
                {tab.label}
              </button>
            </li>
        ...
        <>
          ...
          <ul className="text-gray-500 space-y-2">
            <li>
              <span className="font-medium">Phone: </span> (555) 555 55 55
            </li>
            <li>
              <span className="font-medium">City: </span> Germany / Berlin
            </li>
            <li>
              <span className="font-medium">Birthday: </span> 12/12/1998
            </li>
            <li>
              <span className="font-medium">Work: </span> Web dev
            </li>
            <li>
              <span className="font-medium">Education: </span> HUBT
            </li>
            <li>
              <span className="font-medium">Website: </span>{" "}
              www.franshanscombe.com
            </li>
          </ul>
        </>
      </div>
